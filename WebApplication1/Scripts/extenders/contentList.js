...
$.fn.contentlist = function (options) {
        ...
        $.sendRequest(sittings.reloadServive, data, function (data) {
            ...
            if (data.success) {
                ...
                while (newlist.children().length > 0) {
                    $(newlist.children().get(0)).appendTo(list).execteExtenders();
                }
                ...
                if (sittings.pageSize) {
                    infobar.show().html(list.find('li').length + ' نتيجة من أصل ' + data.totalCount);
    ...
    if (sittings.totalCount) {
        infobar.show().html(list.find('li').length + ' نتيجة من أصل ' + sittings.totalCount);
        //infobar.data('clone').html(list.find('li').length + ' نتيجة من أصل ' + sittings.totalCount).height(infobar.height());
